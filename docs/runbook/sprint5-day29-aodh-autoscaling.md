...
!!! note "這個等式的前提:一個 project ≈ 一種工作負載"
    「租戶邊界 = 資源邊界」在「一個 project 只跑一種負載」時很乾淨。但如果同一個 project 裡混了多種負載(例如又有前端又有批次),`project_id` 這個唯一能圈的維度就太粗——**專案平均會把「某一種負載暴衝」稀釋掉**,結果該擴的沒擴、或被別的負載帶著誤擴。真要按工作負載分別擴縮,得讓它們各自落在不同 project(或換回有 `server_group` 標籤能力的計量後端)。這是 lab 拓撲(換掉 Gnocchi)的約束,不是 Aodh 的理想模型。
