...
[Day 26](sprint4-day26-sprint5-preview.md) 對今天的承諾是「**兩個租戶各拿各的帳單;帳單畫面在 Horizon**」。後者兌現了(見上方畫面),前者打了折:

**其一:雙租戶的對比是退化案例。** team-b 全程沒有任何 VM 或磁碟,所以帳單是 `$0.25 vs $0`。「CloudKitty 能分租戶計費、而且不會把 A 的用量算到 B 頭上」——這個確實證明了;但「兩個租戶都有帳單、金額各自不同、可以互相對照」**沒演到**。要補這個,得在 team-b 開機器再等**兩到三小時**——`period=3600` + `wait_periods=2` 的必然代價。

**其二:儲存計費是紙上談兵。** `volume_size` 的規則寫了、schema 也驗過,但 team-a 和 team-b 都沒有磁碟——**它一毛錢都沒算過**。手上那四條 `volume_size` 序列全屬於別的專案。規則正確與否,今天沒有證據。上面那張 Horizon 畫面裡只有 `instance` 一種計費項,原因就在這。

兩個缺口都不影響今天的核心結論(**分租戶計費的鏈路是通的,而且帳單對得上用量**),但它們是缺口,不是細節。
...
**Day 31** 進入階段四的第一天,也是 Sprint 5 唯一要回多機環境的一天:**Masakari 自癒**——VM 的程序死掉自動重啟,compute 整台掛掉,VM 自動搬到別台去。(內容隨 Sprint 5 進度陸續上線)
