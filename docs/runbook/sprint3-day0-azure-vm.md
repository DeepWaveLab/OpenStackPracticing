...
租一台 VM 看起來只是填幾個欄位,但這台機器要扛完接下來十二天的所有東西——**選錯了不是重開一台就算了,是整個 lab 的架構前提跟著變**。動手前有四個決定要先想通:**機器規格**(要多大、為什麼非得支援巢狀虛擬化)、**Azure 網路約束**(公有雲不給你什麼)、**security type**(為什麼要退回 Standard)、**成本 guardrails**(三道保險絲)。

其中第二項最容易被當成故障:**它不是設定選項,是這個 lab 的物理定律**——後面每一天的網路行為都受它約束,Day 2、Day 8、Day 20 都會再回來找它。

### 1. 機器規格:為什麼是 E16s_v5

先算帳,再挑機型。Kolla-Ansible AIO + Magnum CAPI 全套同時跑的記憶體尖峰(以 Day 8 為準):
...
在 VM 上逐項執行,**全部符合判準才算完成今天**;不符合就回對應步驟追查:
