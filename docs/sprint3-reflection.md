# Sprint 3 回顧 —— Kolla-Ansible + Magnum CAPI:雪恥完成

> Sprint 3(2026-07-07 起,Azure lab)完整回顧。對照 `docs/reflection.md`(Sprint 1)與 `docs/sprint2-course-plan.md`(Sprint 2 OSH),本 sprint **推翻了兩個舊決定並三項雪恥成功**。

## 一句話總結

**Sprint 1 判死刑的三件事(Octavia / Cinder LVM / Magnum),換成 Kolla-Ansible + Magnum CAPI driver 路線後全數活過來,並一路做到 workload cluster 的升版、node group、autoscaler,最後用 Terraform 接管。**
...
| 3 | Cinder LVM(**雪恥 #1 基礎**) | data disk → VG → volume attach、boot from volume |
| 4 | Octavia(**雪恥 #2**) | amphora LB round-robin + OVN provider 對照 |
| 5 | Barbican + Heat | secret 存取往返、HOT stack |
| 6 | CAPI mgmt cluster | kind + clusterctl init + CAPO(v1.13.2/v0.14.4/ORC v2.2.0)|
| 7 | Magnum CAPI driver 整合(**雪恥 #3 前置**) | Kolla 原生 driver 啟用、ClusterTemplate |
| 8 | E2E workload cluster(**雪恥 #3**) | cluster CREATE_COMPLETE、nodes Ready、LoadBalancer→Octavia、PVC→Cinder |
...
## 本 sprint 的真實坑(institutional knowledge)

寫進 `docs/solutions/integration-issues/` 的 6 條,都是文件層級查不到、只有動手才會撞的整合坑:
...
**共同教訓**:單機 + 公有雲(Azure 無 L2)+ 多套系統共存(Kolla/OVN/docker/kind)的組合,坑幾乎都在**元件邊界的網路/資源帳/版本相容**,不在單一元件內部。排查方法論:分層(host→容器→pod)、分類(firewall/NAT vs MTU、config vs 連通性)、以裝好的原始碼/實際錯誤為準而非部落格。
...
1. **雪恥的真正價值不是「成功」,是「每個坑都能定位、能修、能寫下來」** —— Sprint 1 死於無法診斷的 charm/image 斷代(黑箱);Sprint 3 的每個失敗都追到根因並留下 solutions。這才是可累積的能力。
...
- [x] 每個 Day 有 runbook,新坑寫入 `docs/solutions/`(11 份 runbook + 6 條 solutions)
